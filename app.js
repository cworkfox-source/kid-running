...
async function setSetting(id,value){if(id==='reverse'){await writeLocal([{store:'settings',value:{id:`reverse:${ownerId()}`,value,ownerUid:ownerId()}}]);settings=await all('settings');if(backupService&&authUser&&accountState?.backupEnabled)backupService.noteLocalChange(ownerId());return;}await write([{store:'settings',value:{id,value}}]);settings=await all('settings');}
...
try{
 ...
 render();
 attachNetworkHooks();
 if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js').catch(()=>toast('離線快取未啟用；請保持網路連線。'));
