import {today,validate,parseText,summary,speed,improvement,chronological,warnings,csv,validateBackup,LOCAL_OWNER,sameOwner,jsonExportPayload,duplicateGroups,dailyStatistics,chartSeries,repairChildOwnership,pickActiveChild,remapBackupChildIds,chartCaption,uniqueChildId,safeFilenamePart} from './core.js';
...
const sameRecordInput=(old,next)=>['date','distance','seconds','note','startType','surface','timingMethod'].every(key=>old?.[key]===next?.[key]);
...
function chart(points,metric){if(!points.length)return empty('這個範圍沒有資料','調整日期或距離後再試。');const individual=Boolean(points[0]?.individual);const values=points.map(p=>p.value),min=Math.min(...values),max=Math.max(...values),pad=Math.max((max-min)*.15,.1),low=Math.max(0,min-pad),high=max+pad,reverse=metric==='seconds'&&reverseOn(),times=points.map(p=>Date.parse(p.date+'T12:00:00Z')),tmin=Math.min(...times),tmax=Math.max(...times),unit=metric==='seconds'?'秒':'m/s',label=metric==='seconds'?(individual?'秒數':'每日平均秒數'):(individual?'速度 m/s':'平均速度 m/s'),aria=metric==='seconds'?`${distance} 公尺秒數趨勢圖，${reverse?'秒數越低位置越高':'秒數越低位置越低'}`:`${distance} 公尺${label}趨勢圖`,x=i=>50+(tmax===tmin?(points.length===1?.5:i/(points.length-1)):(times[i]-tmin)/(tmax-tmin))*550,y=v=>30+(reverse?(v-low)/(high-low):(high-v)/(high-low))*210;return `<div class="chart-wrap"><svg viewBox="0 0 640 300" role="img" aria-label="${aria}">${Array.from({length:5},(_,i)=>{const v=low+(high-low)*i/4;return `<line x1="50" x2="610" y1="${y(v)}" y2="${y(v)}" stroke="#e6eae9" stroke-dasharray="4 4"/><text x="40" y="${y(v)+4}" text-anchor="end">${v.toFixed(2)}</text>`;}).join('')}<polyline points="${points.map((p,i)=>`${x(i)},${y(p.value)}`).join(' ')}" fill="none" stroke="#188870" stroke-width="3" stroke-linejoin="round"/>${points.map((p,i)=>`<circle cx="${x(i)}" cy="${y(p.value)}" r="5" fill="#188870" stroke="white" stroke-width="2"><title>${esc(p.label||p.date)}：${f(p.value)} ${unit}${!p.individual&&p.count>1?`（${p.count} 次平均）`:''}</title></circle>${i===0||i===points.length-1||points.length<=6?`<text x="${x(i)}" y="270" text-anchor="middle">${p.individual?esc(p.label):short(p.date)}</text>`:''}`).join('')}</svg></div><details><summary>查看圖表數據</summary><table><thead><tr><th>${individual?'次數':'日期'}</th><th>${label}</th>${individual?'':`<th>次數</th><th>當日最佳</th>`}</tr></thead><tbody>${points.map(p=>`<tr><td>${p.individual?esc(p.label):p.date}</td><td>${f(p.value)} ${unit}</td>${individual?'':`<td>${p.count}</td><td>${f(p.bestSeconds)} 秒</td>`}</tr>`).join('')}</tbody></table></details>`;}
function dayStatistics(){const result=dailyStatistics(scopedRecords(),analysisDay,{childId:active()?.id});return `<section class="card"><div class="section-head"><h2>單日統計</h2><label class="filter-label">日期 <input id="analysis-day" type="date" value="${analysisDay}"></label></div>${!result.count?'<p class="muted">這一天尚無紀錄。</p>':`<div class="day-total"><strong>${result.count}</strong><span>次測試</span><span>總距離 ${f(result.totalDistance)}m</span><span>總測試時間 ${f(result.totalSeconds)} 秒</span></div><table><thead><tr><th>距離</th><th>次數</th><th>平均秒數</th><th>最佳秒數</th><th>平均速度</th></tr></thead><tbody>${result.byDistance.map(g=>`<tr><td>${g.distance}m</td><td>${g.count}</td><td>${f(g.averageSeconds)} 秒</td><td>${f(g.bestSeconds)} 秒</td><td>${f(g.averageSpeed)} m/s</td></tr>`).join('')}</tbody></table><p class="quiet">平均速度為該日每次速度的平均，不以平均秒數反推。</p>`}</section>`;}
function analysis(){const s=stat(),points=chartSeries(scopedRecords(),{childId:active()?.id,distance:Number(distance),start:analysisStart,end:analysisEnd,metric:analysisMetric}),caption=chartCaption({metric:analysisMetric,start:analysisStart,end:analysisEnd});return `<section class="card"><div class="section-head"><h2>成績趨勢</h2>${distanceSelect()}</div><div class="analysis-filters"><label>開始日期<input id="analysis-start" type="date" value="${analysisStart}"></label><label>結束日期<input id="analysis-end" type="date" value="${analysisEnd}"></label><label>顯示<select id="analysis-metric">${options([['seconds','秒數'],['ms','平均速度 m/s']],analysisMetric)}</select></label></div><div class="analysis-filter-actions"><button type="button" class="secondary" id="apply-analysis-range">套用日期區間</button><button type="button" class="secondary" id="clear-analysis-range" ${analysisStart||analysisEnd?'':'disabled'}>清除日期區間</button></div><p class="muted">${esc(caption)}</p>${analysisMetric==='seconds'?`<label class="checkbox"><input id="reverse" type="checkbox" ${reverseOn()?'checked':''}>反轉 Y 軸，讓進步向上</label>`:''}${chart(points,analysisMetric)}</section>${dayStatistics()}${s?`<section class="card analysis-summary"><h2>${distance} 公尺 · 進步摘要</h2>${summaryHTML(s)}</section>`:''}<section class="card"><h2>個人最佳紀錄</h2>${own().length?`<div class="pb-grid">${[...new Set(own().map(r=>r.distance))].sort((a,b)=>a-b).map(d=>{const b=summary(scopedRecords(),d,active().id).best;return `<div><span>${d} 公尺</span><strong>${f(b.seconds)}<small> 秒</small></strong><span>${b.date}</span></div>`;}).join('')}</div>`:'<p class="muted">第一筆成績會成為你的起始紀錄。</p>'}<p class="quiet">手動計時、場地與起跑方式都可能影響成績，建議在相近條件下觀察趨勢。</p></section>`;}
...
<section class="card"><h2>本機 JSON／CSV</h2><div class="notice">資料仍以這台裝置的 IndexedDB 為日常來源。清除網站資料或更換手機前，建議再匯出一份 JSON。</div>${exportFallbackHTML()}<div class="settings-row"><div><strong>JSON 完整備份</strong><p>保留目前帳號的紀錄、小孩與必要偏好</p></div><button class="secondary" id="export-json">匯出 JSON</button></div><div class="settings-row"><div><strong>${esc(active()?.name||'小孩')}的 CSV 成績表</strong><p>只匯出目前小跑者，供 Excel、Google Sheets 或分析使用</p></div><button class="secondary" id="export-csv">匯出 ${esc(active()?.name||'小孩')} CSV</button></div><div class="settings-row"><div><strong>還原 JSON 備份</strong><p>先檢查內容，再確認取代目前帳號的本機資料</p></div><label class="secondary file-label">選擇檔案<input id="import-json" type="file" accept=".json,application/json"></label></div><div id="backup-preview"></div><p class="quiet">目前這個帳號共 ${scopedRecords().length} 筆紀錄。${settings.find(s=>s.id==='lastBackup')?`上次匯出：${esc(new Date(settings.find(s=>s.id==='lastBackup').value).toLocaleString('zh-TW'))}`:'尚未匯出備份。'}</p></section>
...
function contextNow(){return {ownerUid:ownerId(),childId:active()?.id||null};}
function markDraftContext(){draftContext??=contextNow();}
function capture(){const wasEmpty=!hasUnsavedChildWork();for(const k of ['date','distance','seconds','note','startType','surface','timingMethod'])if($('#'+k))draft[k]=$('#'+k).value;if(wasEmpty&&hasUnsavedChildWork())markDraftContext();}
...
async function ensureChild(){
 ...
 for(const child of repaired.children){
  const prev=childById.get(child.id);
  if(!prev||prev.ownerUid!==child.ownerUid||prev.name!==child.name||prev.birthday!==child.birthday)writes.push({store:'children',value:child});
 }
 const recById=new Map(records.map(r=>[r.id,r]));
 for(const rec of repaired.records){
  const prev=recById.get(rec.id);
  if(prev&&prev.childId!==rec.childId)writes.push({store:'records',value:rec});
 }
...
async function writeLocal(changes){if(authInfo.resolving)throw Error('帳號狀態讀取中，請稍候再儲存');const uid=ownerId();await write(changes.map(c=>c.value&&(c.store==='records'||c.store==='children')?{...c,value:{...c.value,ownerUid:uid}}:c),{bumpRevision:true,ownerUid:uid});if(!replacingData&&backupService&&authUser?.uid===uid&&accountState?.backupEnabled)backupService.noteLocalChange(uid);}
...
function resetChildDraft(){editing=null;preview=[];pasteDraft='';duplicateScanOpen=false;draftContext=null;draft={date:today(),distance,seconds:'',note:'',startType:'',surface:'',timingMethod:''};}
...
function confirmDraftContext(){if(!hasUnsavedChildWork()||!draftContext)return true;const current=contextNow();if(current.ownerUid===draftContext.ownerUid&&current.childId===draftContext.childId)return true;if(!confirm('帳號或小孩已變更。要將目前未儲存內容改為記錄到現在的小跑者嗎？'))return false;draftContext=current;return true;}
function confirmDiscardChildWork(){if(page==='home')capture();return !hasUnsavedChildWork()||confirm('目前有尚未儲存的內容。繼續後將不保留這些內容，仍要繼續嗎？');}
async function switchActiveChild(id){
 ...
 if(!confirmDiscardChildWork()){render();return;}
...
function bind(){document.querySelectorAll('[data-page]').forEach(b=>b.onclick=()=>changePage(b.dataset.page));$('.brand').onclick=e=>{e.preventDefault();changePage('home');};$('#child-picker')?.addEventListener('click',()=>changePage('settings'));
 ...
 editing=null;draftContext=null;distance=r.distance;draft={...draft,date:today(),distance:r.distance,seconds:'',note:'',startType:'',surface:'',timingMethod:''};await setSetting('distance',r.distance);render();toast(message);$('#seconds').focus();});};$('#parse').onclick=()=>{markDraftContext();preview=parseText($('#paste').value);$('#preview').innerHTML=previewHTML();bindPreview();if(!preview.length)toast('請先貼上跑步紀錄');};bindPreview();}
 ...
 const applyAnalysisRange=()=>{const start=$('#analysis-start')?.value||'',end=$('#analysis-end')?.value||'';if(start&&end&&start>end){toast('開始日期不可晚於結束日期');return;}analysisStart=start;analysisEnd=end;render();};
 $('#apply-analysis-range')?.addEventListener('click',applyAnalysisRange);
 $('#clear-analysis-range')?.addEventListener('click',()=>{analysisStart='';analysisEnd='';render();});
 ...
 document.querySelectorAll('[data-edit],[data-copy]').forEach(b=>b.onclick=()=>{const r=records.find(x=>x.id===(b.dataset.edit||b.dataset.copy));editing=b.dataset.edit?r.id:null;draft={...r,date:b.dataset.copy?today():r.date};draftContext=contextNow();page='home';render();window.scrollTo(0,0);$('#seconds').focus();if(b.dataset.copy)toast('已複製到輸入欄，確認後再新增');});document.querySelectorAll('[data-delete]').forEach(b=>b.onclick=()=>safe(async()=>{const r=records.find(x=>x.id===b.dataset.delete);if(!confirm(`刪除 ${r.date} · ${r.distance} 公尺 ${r.seconds} 秒？\n刪除後本機無法復原，除非已有 JSON 或雲端版本。`))return;await writeLocal([{store:'records',delete:r.id}]);await refresh();render();toast('已刪除這筆紀錄；既有雲端成功版本仍會保留。');}));
 ...
 $('#add-child-form')?.addEventListener('submit',e=>{e.preventDefault();safe(async()=>{const name=$('#new-child-name').value.trim();if(!name)throw Error('名稱不可空白');if(!confirmDiscardChildWork())return;const uid=ownerId();const id=uniqueChildId(uid,new Set(children.map(child=>child.id)));await writeLocal([{store:'children',value:{id,name,birthday:null,ownerUid:uid}},{store:'settings',value:{id:`active-child:${uid}`,value:id}}]);await refresh();resetChildDraft();render();toast(`已新增小跑者：${name}`);});});
 ...
 $('#export-csv')?.addEventListener('click',()=>safe(async()=>{await exportCurrentFile(`run-data-${safeFilenamePart(active()?.name)}-${today()}.csv`,csv(own()),'text/csv;charset=utf-8');}));
...
function bindPreview(){if($('#paste')){$('#paste').value=pasteDraft;$('#paste').oninput=e=>{markDraftContext();pasteDraft=e.target.value;preview=[];$('#preview').innerHTML='';};}document.querySelectorAll('[data-preview]').forEach(input=>input.oninput=()=>{markDraftContext();preview[Number(input.dataset.preview)].record[input.dataset.field]=input.dataset.field==='date'?input.value:Number(input.value);});$('#confirm-import')?.addEventListener('click',()=>safe(async()=>{if(preview.some(x=>x.error)||!preview.length||!confirmDraftContext())return;const incoming=preview.map((p,i)=>makeRecord(validate(p.record),i));if(!await saveRecords(incoming))return;preview=[];pasteDraft='';draftContext=null;capture();render();toast(`已匯入 ${incoming.length} 筆紀錄`);}));}
