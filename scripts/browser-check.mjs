...
try{
 ...
 await nav('analysis');assert.equal(await page.locator('svg circle').count(),4);await page.locator('#reverse').uncheck();await page.waitForFunction(()=>document.querySelector('svg').getAttribute('aria-label').includes('位置越低'));
 await page.screenshot({path:join(artifactDir,'mobile-analysis.png'),fullPage:true});
 await nav('settings');assert.ok(await page.locator('#drive-backup').isVisible());assert.ok(await page.locator('#drive-restore').isVisible());const dl=page.waitForEvent('download');await page.locator('#export-json').click();const download=await dl;await download.saveAs(join(artifactDir,'test-backup.json'));
 ...
 assert.deepEqual(errors,[]);console.log('PASS: mobile CRUD, preview, PB, persistence, backup restore, invalid import, responsive 320–1280px, offline reload/write; no browser errors.');
